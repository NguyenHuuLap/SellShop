...
export const setCategory = (category) => ({
  ...
  export const searchProducts = (minPrice, maxPrice, category, brand, keyword, page = 1) => {
    return (dispatch) => {
      axios.get(`http://localhost:3030/product/search?minPrice=${minPrice}&maxPrice=${maxPrice}&category=${category}&brand=${brand}&keyword=${keyword === '' ? '' : encodeURIComponent(keyword)}&page=${page}`)
        .then(response => {
          dispatch({
            type: 'SEARCH_PRODUCTS',
