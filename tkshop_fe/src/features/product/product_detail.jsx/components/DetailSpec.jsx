...
const DetailSpec = ({ open, setOpen, detailSpecs }) => {
    ...
    return !detailSpecs ? (<>Loading</>) : (
        ...
        >
            ...
            <DialogContent>
                <DialogContentText>
                    {detailSpecs.map((item, index) => {
                        return (
                            <>
                                <Typography variant="body1" fontWeight={700} sx={{ color: "black", mt: 2 }}>
                                    {item.groupName}
                                </Typography>
            ...
            <DialogActions>
                <Button variant='contained' sx={{ width: "100%", m: 1.5, backgroundColor: "rgb(41, 182, 246)" }} autoFocus startIcon={<HighlightOffIcon />}
                    onClick={handleClose}
                >
