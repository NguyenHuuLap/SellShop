...
=======
import { AccordionDetails, AccordionSummary, Accordion, FormHelperText, Paper, Grid, Typography, Box, Card, CardContent, FormGroup, FormControlLabel, FormLabel, Checkbox, RadioGroup, Radio, ToggleButton, ToggleButtonGroup, Stack, FormControl, Slider, Divider, Input } from "@mui/material";
import * as React from "react";
import ProductGrid from "../components/ProductGrid";
import { styled } from "@mui/styles";
import CheckCircleIcon from '@mui/icons-material/CheckCircle';
...
<<<<<<< HEAD
  ...
  const handleChange = (event) => {
    ...
    const updateValue = tempList.map((item) => ({
      ...item,
      checked:
        event.target.name === item.key ? event.target.checked : item.checked,
    }));
    setDataSearch(updateValue);
=======
    React.useEffect(() => {
        const fecthData = async () => {
            try {
                ...
                const data1 = response.data.map(item => ({ ...item, checked: false }));
                setListBrand(data1);
                console.log(data1);
...
<<<<<<< HEAD
  ...
  return (
    <Box>
      <Typography sx={{ fontSize: 16, fontWeight: 600, color: "black", mb: 1 }}>
        {title}
      </Typography>
...
=======
    const handleChange = (event) => {
        ...
        const updateValue = tempList.map(item => ({
            ...item,
            checked: event.target.name === item.slug ? event.target.checked : item.checked,
        }));
        ...
        navigate(`/search?category=${category}&minPrice=${minPrice}&maxPrice=${maxPrice}&brand=${textParams}&keyword=${encodeURIComponent(keyword)}&page=1`)
    }
        ...
        <Box>
            <Accordion sx={{boxShadow: "none"}}>
                <AccordionSummary
                    ...
                    id="panel1a-header"
                >
                    <Typography sx={{ fontSize: 16, fontWeight: 600, color: "black"}}>{title}</Typography>
                </AccordionSummary>
                <AccordionDetails sx>
                    <FormControl component="fieldset" variant="standard">
                        <FormGroup>
                            {listBrand.map((item, index) => {
                                return (
                                    <FormControlLabel key={index}
                                        control={
                                            <Checkbox sx={{py: 0}} checked={item.checked} onChange={handleChange} name={item.slug} />
                                        }
                                        label={item.name}
                                        sx={{my: 0, py: 0}}
                                    />
