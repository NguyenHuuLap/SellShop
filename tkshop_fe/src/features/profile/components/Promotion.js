...
const PromotionButtons = ({ title, buttonName, activeButton, path, onClick }) => (
  <Button
    sx={{
      py: 0.5,
      textTransform: 'none',
...
const TkMemberButtons = ({ title, buttonName, activeButton, path, onClick }) => (
    ...
    <>
      <Grid container item sx={{ position: 'relative' }} spacing={3} direction='row'>
        <Grid item sx={{ position: 'relative', alignItems: 'center' }}>
          <Box sx={{ width: '900px', backgroundColor: '#7676801f', borderRadius: '10px', marginBottom: '20px' }}>
