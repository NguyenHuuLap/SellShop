...
export const addItem = async(req, res, next) => {
    try{
        ...
        const addResult = await cartService.addItems(userId, productId, sku, quantity);
        if(addResult){
            res.status(200).json({message: 'Add item to cart successfully', data: addResult});
        }else{
...
export const updateItemQuantity = async(req, res, next) =>{
    try{
        ...
        const updateResult = await cartService.updateItemQuantity(userId, productId, sku, delta);
        if(updateResult){
            res.status(200).json({message: 'update quantity item in cart successfully', data: updateResult})
        }
        else{
            res.status(500).json({message: 'has error update quantity', data: updateResult});
        }
    }catch(err){
        next(err);
    }
