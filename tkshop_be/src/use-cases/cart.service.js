...
const getProductInfo = async (productId, sku, notClean =true) =>{
    ...
    if(!variant){
        throw ApiErrorUtils.simple(`Variant sku=${sku} of product ${productId} not found`,
          404);
    }
...
const addItems = async(userId, productId, sku, quantity=1) =>{
    ...
    if(!userCart){
        ...
        userCart = await newCart.save();
            ...
            const itemInCart = userCart.items.find(i => i.productId.toString() === productId && i.sku !== undefined && i.sku === sku);
            if (itemInCart) {
...
const updateItemQuantity = async(userId, productId, sku, delta) =>{
    ...
    const item = await userCart?.items?.find(i => i.productId.toString() === productId && i.sku ===sku);
    if(!userId || !item){
        throw ApiErrorUtils.simple(`This is not exits in your cart`, 404);
    }
    item.quantity += parseInt(delta);
    if(item.quantity <=0){
        return cartModel.findByIdAndUpdate(userCart._id, {$pull: {items: {productId, sku}}}, {new: true});
    }else{
...
export default {getCartItemsFromData,getCartItemsByUser, updateItemQuantity, addItems, removeItem, cleanCart}